...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_retrieve_for_auth_user_only(self):
    ...
    def test_create_tag_on_update(self):
        ...
        recipe = create_recipe(user=self.user)
        payload = { 'tags' : [{"name" : "Tagname"}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url,payload,format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)

        new_tag  = Tag.objects.get(user= self.user,name="Tagname")
    ...
    def test_clearing_a_recipe_tag_on_update(self):

        tag = Tag.objects.create(user = self.user, name = "tagsample")
        recipe = create_recipe(user = self.user)
        recipe.tags.add(tag)
    ...
    def test_create_ingredient_on_update(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        ingredient_added = Ingredient.objects.get(user = self.user, name = "Sugar")
        self.assertIn(ingredient_added, recipe.ingredients.all())
